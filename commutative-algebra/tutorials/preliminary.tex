...
\begin{proof}
  ...
  \operatorname{ker}\beta_{\mathfrak{m}} \), 从而由交换图
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	0 & {(\operatorname{ker} \beta)_{\mathfrak{m}}} & {M_{\mathfrak{m}}} & {M''_{\mathfrak{m}}} & 0 \\
	0 & {(\operatorname{ker} \beta_{\mathfrak{m}})} & {M_{\mathfrak{m}}} & {M''_{\mathfrak{m}}} & 0
	\arrow[from=1-1, to=1-2]
	\arrow[from=1-2, to=1-3]
	\arrow[from=1-2, to=2-2]
	\arrow[from=1-3, to=1-4]
	...
	\arrow[from=2-4, to=2-5]
\end{tikzcd}\]
  知道 \( (\operatorname{ker}\beta)_{\mathfrak{m}} =
  \operatorname{ker}\beta_{\mathfrak{m}} \). 同理 \(
  (\operatorname{im}\alpha)_{\mathfrak{m}} = \operatorname{im}
  \alpha_{\mathfrak{m}} \), 因此 \( \left(\frac{\operatorname{ker} \beta +
...
\section{有向极限与逆极限}
给定一个预序集范畴 \( (I, \leq) \), 给定范畴 \( \mathcal{M} \) 其对象集是模 \(
(M_i)_{i \in I} \), 态射集为 \( \left\lbrace M_i \to M_j: i \leq j \right\rbrace
\), 我们称模 \( \varinjlim M_k \) 为逗号范畴 \( \mathbf{1} \xrightarrow{\alpha}
\mathcal{M}^I = \operatorname{Fct}(I, \mathcal{M}) \xleftarrow{\Delta}
\mathcal{M} \) 的始对象为 \emph{有向极限}, 其中 \( \alpha \) 为函子 \(
\operatorname{pt} \mapsto \left( (i \mapsto j) \mapsto (M_i \to M_j) \right) \)
, \( \Delta \) 为对角函子. 换句话说, \( \varinjlim M_k \) 满足下面的交换图.
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	{M_i} && {M_j} \\
	& {\varinjlim M_k} \\
	& N
